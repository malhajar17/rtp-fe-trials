...
name = ""
# Only show title and motto if the user is not logged in
if not authentication_status:
    ...
    # Sign up button if the user is not authenticated
    if auth.check_signup():
        st.markdown('<div class="auth-container">', unsafe_allow_html=True)
        auth.sign_up(authenticator,credentials)  # Sign-up form
        st.markdown('</div>', unsafe_allow_html=True)
else:
    # Once logged in, hide the title and sign-up button and display the rest of the app
